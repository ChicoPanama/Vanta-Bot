...
class BulletproofAvantisTrader:
    ...
    def __init__(self):
        ...
        self.TRADING_PROXY = (
            "0x44914408af82bC9983bbb330e3578E1105e11d4e"  # Active proxy
        )
        ...
        self.trading_contract = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.TRADING_PROXY), abi=self.TRADING_ABI
        )
        ...
        logger.info("🚀 BULLETPROOF TRADER INITIALIZED")
    ...
    def check_pause_status(self) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            if max_slippage:
                ...
                logger.info(
                    f"   Max Slippage: {max_slippage:,} ({max_slippage_pct:.2f}%)"
                )
    ...
    def validate_on_chain_limits(
        self, scaled_params: dict[str, int], pair_index: int = 0
    ) -> tuple[bool, str]:
        """Validate against on-chain limits."""

        logger.info("🔍 VALIDATING ON-CHAIN LIMITS")
        ...
        try:
            ...
            if scaled_params["slippage_scaled"] > limits["max_slippage"]:
                return (
                    False,
                    f"Slippage {scaled_params['slippage_scaled']} exceeds max {limits['max_slippage']}",
                )

            if scaled_params["position_size_usdc"] <= 0:
                return False, "Position size must be greater than 0"

            logger.info("   ✅ All limits validated successfully")
            return True, "OK"
    ...
    def staticcall_validation(
        ...
        try:
            ...
            # Make static call
            result = self.w3.eth.call({"to": self.TRADING_PROXY, "data": data})
    ...
    def watch_unpaused_event(self, start_block: Optional[int] = None) -> bool:
        ...
        logger.info("👀 WATCHING FOR UNPAUSED EVENT")
            ...
            try:
                ...
                if latest > last_block:
                    ...
                    if logs:
                        ...
                        logger.info(
                            f"🔔 UNPAUSED EVENT DETECTED at block {unpaused_block}"
                        )
                        return True
    ...
    def execute_trade(
        ...
        try:
            ...
            scaled_params = self.manual_scale_parameters(
            ...
            is_valid, validation_msg = self.validate_on_chain_limits(
                scaled_params, pair_index
            )
            if not is_valid:
                return {
                    "success": False,
                    "error": f"Limit validation failed: {validation_msg}",
                }
    ...
    def run_bulletproof_test(self):
        ...
        logger.info("🧪 RUNNING BULLETPROOF TEST")
        logger.info("=" * 60)
        ...
        if result["success"]:
            ...
            logger.info("   Results saved to: bulletproof_trade_results.json")
            ...
            logger.info("   Error details saved to: bulletproof_trade_error.json")
