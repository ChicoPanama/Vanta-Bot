...
async def debug_exact_parameters():
    ...
    try:
            ...
            try:
                ...
                if hasattr(build_error, "args") and len(build_error.args) > 0:
                    ...
                    if "BELOW_MIN_POS" in error_msg:
                        print("   → BELOW_MIN_POS error - position size too small")
                        print(f"   → Current position size: {position_size} USDC")
                        print(
                            f"   → This suggests minimum is higher than {position_size} USDC"
                        )
                    else:
        ...
        print(
            "   Go to: https://basescan.org/address/0x44914408af82bC9983bbb330e3578E1105e11d4e"
        )
        print("   Look for recent openTrade transactions")
        print("   Check what parameters they used")
        print("   Compare with our parameters above")

        print("\n✅ DEBUG COMPLETE!")
        print("✅ Check the output above for:")
        print("  - Exact parameters being sent")
        print("  - Pair details and indexes")
        print("  - Build error details")
        print("  - Comparison with successful trades")
