...
def test_simple():
    ...
    print(
        f"Input: ${collateral_usdc} USDC, {leverage_x}x leverage, {slippage_pct}% slippage"
    )

    # Use refactored scaling
    trade_units = to_trade_units(collateral_usdc, leverage_x, slippage_pct)

    print("Refactored Scaling Results:")
    ...
    if trade_units.position_size_usdc == expected_position:
        ...
        print("❌ Scaling invariant failed")
        return False
    ...
    try:
        ...
        # Perform static call
        w3.eth.call({"to": TRADING_PROXY, "data": data, "from": WALLET_ADDRESS})
