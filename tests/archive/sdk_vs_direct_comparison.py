...
def demonstrate_scaling_issue():
    ...
    position_size_correct = (
        initial_pos_token_correct * leverage_scaled_correct
    ) // int(SCALE_1E10)
    ...
    logger.info("  SDK double-scales to:")
    ...
    logger.info("\n💥 THE DAMAGE:")
    logger.info(
        f"  Collateral: {collateral_usdc} → {double_scaled_collateral / USDC_6:.2f} USDC"
    )
...
def show_validation_issues():
    ...
    if double_scaled > max_slippage:
        logger.error(
            f"❌ Double-scaled slippage EXCEEDS max by {double_scaled - max_slippage:,}"
        )
...
def main():
    ...
    logger.info("\n🚀 Next steps:")
    logger.info("  1. Run: python practical_direct_contract_test.py")
    logger.info("  2. Compare results with your SDK tests")
    logger.info("  3. If direct approach works, you've proven the double-scaling issue")
