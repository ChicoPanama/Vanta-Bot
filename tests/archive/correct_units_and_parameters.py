    ...
    try:
        ...
        try:
            ...
            if hasattr(trading_contract.functions, 'minPositionSizeUSDC'):
                min_pos = await trading_contract.functions.minPositionSizeUSDC(pair_index).call()
            ...
            if hasattr(trading_contract.functions, 'maxPositionSizeUSDC'):
                max_pos = await trading_contract.functions.maxPositionSizeUSDC(pair_index).call()
        ...
        for human_collateral, human_leverage, human_slippage in test_configs:
            ...
            try:
                ...
                initialPosToken = int(Decimal(human_collateral) * Decimal(10**6))  # 6 dp
                leverage_scaled = int(Decimal(human_leverage) * Decimal(10**10))  # 1e10 scale
                slippage_scaled = int((Decimal(human_slippage) / Decimal(100)) * Decimal(10**10))  # 1e10 of fraction
                
                # Compute position size in USDC (6 dp)
                positionSizeUSDC = (initialPosToken * leverage_scaled) // (10**10)
                
                print(f'   📊 Protocol units:')
                print(f'   initialPosToken: {initialPosToken} (${human_collateral} USDC)')
        ...
        print(f'❌ Even with correct units failed')
