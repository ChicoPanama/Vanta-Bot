...
async def test_market_intelligence_workflow(mock_services):
    ...
    with patch.object(market_intelligence, '_fetch_pyth_price') as mock_fetch:
        ...
        await market_intelligence._update_regime_analysis('BTC-USD', mock_fetch.return_value)
...
async def test_event_indexing_workflow(mock_services):
    ...
    with patch.object(event_indexer, 'web3') as mock_web3:
        ...
        mock_web3.eth.get_block.return_value = {'timestamp': int(datetime.utcnow().timestamp())}
        ...
        with patch.object(event_indexer.trading_contract.events, 'TradeOpened') as mock_events:
...
async def test_position_tracking_workflow(mock_services):
    ...
    stats = await position_tracker._calculate_trader_stats('0xabc123', datetime.utcnow() - timedelta(days=30))
