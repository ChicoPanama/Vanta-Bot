...
from src.ai.trader_analyzer import TraderAnalysis, TraderAnalyzer
...
class TestTraderAnalyzer:
    ...
    def test_optimal_copy_ratio_calculation(self):
        ...
        good_metrics = {"sharpe_like": 1.2, "max_drawdown": 0.08, "consistency": 0.8}
        ratio = analyzer._calculate_optimal_copy_ratio(good_metrics)
        assert 0.01 <= ratio <= 0.5  # Should be within reasonable range

        # Test with poor metrics
        poor_metrics = {"sharpe_like": -0.5, "max_drawdown": 0.3, "consistency": 0.2}
...
pytestmark = pytest.mark.skipif(
    not os.getenv("RUN_SLOW"), reason="slow/infra-heavy suite; set RUN_SLOW=1 to enable"
)
