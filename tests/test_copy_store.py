...
from src.services.copytrading.copy_store import (
    init, get, put, remove, list_follows, users_by_trader, all_trader_keys, DEFAULT_CFG
)
...
def test_init(temp_db):
    ...
    try:
        cur = con.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='user_follow_configs'")
...
def test_index_performance(temp_db):
    ...
    con = sqlite3.connect(temp_db)
    try:
        cur = con.execute("EXPLAIN QUERY PLAN SELECT user_id FROM user_follow_configs WHERE trader_key=?", (trader_key,))
        plan = cur.fetchone()[3]
