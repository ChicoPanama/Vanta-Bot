"""Working oracle facade testing with proper async mocks."""
...
class TestOracleFacadeWorking:
    """Working oracle facade testing."""
    
    @pytest.fixture
    def mock_primary_provider(self):
        """Mock primary provider with async get_price."""
        provider = Mock()
    ...
    def mock_secondary_provider(self):
        """Mock secondary provider with async get_price."""
        provider = Mock()
        provider.get_price = AsyncMock(return_value=PriceQuote(
    ...
    @pytest.mark.asyncio
    async def test_healthy_dual_feed_operation(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
    ...
    @pytest.mark.asyncio
    async def test_primary_failure_fallback(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
    ...
    @pytest.mark.asyncio
    async def test_high_deviation_rejection(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
    ...
    @pytest.mark.asyncio
    async def test_stale_price_rejection(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
    ...
    @pytest.mark.asyncio
    async def test_symbol_normalization(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
        ...
        assert mock_secondary_provider.get_price.call_count == 2
    ...
    @pytest.mark.asyncio
    async def test_concurrent_price_requests(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
        ...
        assert mock_secondary_provider.get_price.call_count == 3
    
    @pytest.mark.asyncio
    async def test_provider_timeout_handling(self, mock_primary_provider, mock_secondary_provider):
        """Test timeout handling in oracle facade."""
        # Make primary timeout
        mock_primary_provider.get_price.side_effect = asyncio.TimeoutError("Request timeout")
    ...
    @pytest.mark.asyncio
    async def test_both_providers_fail(self, mock_primary_provider, mock_secondary_provider):
        """Test behavior when both providers fail."""
        # Make both providers fail
        mock_primary_provider.get_price.side_effect = Exception("Primary unavailable")
        mock_secondary_provider.get_price.side_effect = Exception("Secondary unavailable")
        ...
        # Should raise aggregated failure when both providers fail
        with pytest.raises(Exception, match="Both oracle providers failed"):
            await facade.get_price('BTC/USD')
    
    @pytest.mark.asyncio
    async def test_price_deviation_calculation(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
    ...
    @pytest.mark.asyncio
    async def test_unsupported_symbol_handling(self, oracle_facade, mock_primary_provider, mock_secondary_provider):
        """Test handling of unsupported symbols."""
        # Make providers raise unsupported symbol error
        mock_primary_provider.get_price.side_effect = ValueError("Unsupported symbol: UNKNOWN")
        mock_secondary_provider.get_price.side_effect = ValueError("Unsupported symbol: UNKNOWN")
