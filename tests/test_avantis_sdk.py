...
from src.services.markets.avantis_price_provider import AvantisPriceProvider, TradeQuote
from src.services.trading.avantis_executor import AvantisExecutor, OrderRequest
...
class TestOrderRequest:
    ...
    def test_order_request_defaults(self):
        ...
        order = OrderRequest(
            pair="BTC/USD", is_long=False, collateral_usdc=50.0, leverage=5.0
        )
...
class TestAvantisPriceProvider:
    ...
    async def test_estimate_opening_fee_mock(self, price_provider):
        ...
        with patch.object(price_provider, "_get_client") as mock_get_client:
            ...
            mock_client.fee_parameters.get_opening_fee.assert_called_once_with(
                trade_input
            )
    ...
    async def test_quote_open_mock(self, price_provider):
        ...
        with (
            patch.object(price_provider, "get_pair_index", return_value=1),
            patch.object(price_provider, "estimate_opening_fee", return_value=2.5),
            patch.object(price_provider, "get_loss_protection") as mock_loss_protection,
            patch.object(price_provider, "get_pair_spread", return_value=None),
            patch.object(price_provider, "get_price_impact_spread", return_value=0.1),
            patch.object(price_provider, "get_skew_impact_spread", return_value=0.05),
        ):
...
class TestIntegration:
    ...
    async def test_mock_trade_execution_flow(self):
        ...
        order = OrderRequest(
            pair="ETH/USD", is_long=True, collateral_usdc=100.0, leverage=10.0
        )
        ...
        with (
            patch.object(executor, "_get_sdk_client") as mock_get_client,
            patch.object(executor, "_get_price_provider") as mock_get_provider,
        ):
            mock_client = Mock()
            mock_client.get_trader_address.return_value = (
            ...
            mock_client.sign_and_get_receipt = AsyncMock(
