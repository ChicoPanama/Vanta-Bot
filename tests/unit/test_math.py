...
from src.core.math import (
    ...
    from_trade_units,
    get_max_safe_leverage,
    to_trade_units,
    validate_scaling_invariant,
)
...
class TestScalingFunctions:
    ...
    def test_from_trade_units_roundtrip(self):
        ...
        trade_units = to_trade_units(
            original_collateral, original_leverage, original_slippage
        )
...
class TestLiquidationCalculation:
    ...
    def test_high_leverage_liquidation(self):
        ...
        assert abs(long_liquidation - entry_price) < entry_price * Decimal(
            "0.02"
        )  # Within 2%
        assert abs(short_liquidation - entry_price) < entry_price * Decimal(
            "0.02"
        )  # Within 2%
