...
class TestPnLCalculation:
    ...
    def test_invalid_side_raises_error(self):
        ...
        with pytest.raises(ValueError):
            calculate_pnl(
                Decimal("50000"), Decimal("51000"), Decimal("1000"), "invalid"
            )
...
class TestMarginRatio:
    ...
    def test_long_margin_ratio_decreases_with_price_drop(self):
        ...
        # Price at entry
        margin_at_entry = calculate_margin_ratio(
            entry_price, entry_price, leverage, "long"
        )
        assert margin_at_entry == Decimal("1")  # 100% margin at entry

        # Price drops
        margin_at_lower = calculate_margin_ratio(
            entry_price, Decimal("45000"), leverage, "long"
        )
    ...
    def test_short_margin_ratio_decreases_with_price_rise(self):
        ...
        # Price at entry
        margin_at_entry = calculate_margin_ratio(
            entry_price, entry_price, leverage, "short"
        )
        assert margin_at_entry == Decimal("1")  # 100% margin at entry

        # Price rises
        margin_at_higher = calculate_margin_ratio(
            entry_price, Decimal("55000"), leverage, "short"
        )
...
class TestRiskScore:
    ...
    def test_risk_score_increases_near_liquidation(self):
        ...
        liq_price = calculate_liquidation_price(
            entry_price, leverage, Decimal("0.001"), Decimal("0.05"), "long"
        )
