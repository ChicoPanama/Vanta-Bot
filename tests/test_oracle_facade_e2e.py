...
class TestOracleFacadeE2E:
    ...
    @pytest.fixture
    def oracle_facade(
        self, mock_pyth_provider, mock_chainlink_provider, price_validator
    ):
        """Create oracle facade with mocked providers."""
        return OracleFacade(
            primary=mock_pyth_provider, secondary=mock_chainlink_provider
        )

    @pytest.mark.asyncio
    async def test_healthy_dual_feed_operation(
        self, oracle_facade, mock_pyth_provider, mock_chainlink_provider
    ):
    ...
    async def test_provider_timeout_handling(
        ...
        mock_pyth_provider.get_price.side_effect = asyncio.TimeoutError(
            "Request timeout"
        )
    ...
    async def test_both_providers_fail(
        ...
        mock_pyth_provider.get_price.side_effect = Exception("Pyth unavailable")
        mock_chainlink_provider.get_price.side_effect = Exception(
    ...
    async def test_unsupported_symbol_handling(
        ...
        mock_pyth_provider.get_price.side_effect = ValueError(
            "Unsupported symbol: UNKNOWN"
        )
        mock_chainlink_provider.get_price.side_effect = ValueError(
            "Unsupported symbol: UNKNOWN"
        )
