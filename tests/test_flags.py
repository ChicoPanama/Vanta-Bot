...
from src.config.flags import ExecutionMode, FeatureFlags, flags
...
class TestFeatureFlags:
    ...
    def test_can_show_leaderboard(self):
        ...
        # Normal conditions
        with patch.dict(
            os.environ, {"MAINTENANCE_MODE": "false", "ENABLE_LEADERBOARD": "true"}
        ):
            flags_obj = FeatureFlags()
        ...
        # Maintenance mode
        with patch.dict(
            os.environ, {"MAINTENANCE_MODE": "true", "ENABLE_LEADERBOARD": "true"}
        ):
            flags_obj = FeatureFlags()
        ...
        with patch.dict(
            os.environ, {"MAINTENANCE_MODE": "false", "ENABLE_LEADERBOARD": "false"}
        ):
            flags_obj = FeatureFlags()
            assert flags_obj.can_show_leaderboard() is False
    ...
    def test_can_process_telegram_commands(self):
        ...
        with patch.dict(
            os.environ,
            {"MAINTENANCE_MODE": "false", "ENABLE_TELEGRAM_COMMANDS": "true"},
        ):
            flags_obj = FeatureFlags()
        ...
        # Maintenance mode
        with patch.dict(
            os.environ, {"MAINTENANCE_MODE": "true", "ENABLE_TELEGRAM_COMMANDS": "true"}
        ):
            flags_obj = FeatureFlags()
    ...
    def test_get_slippage_tolerance(self):
        ...
        # Live mode
        with patch.dict(
            os.environ, {"COPY_EXECUTION_MODE": "LIVE", "DEFAULT_SLIPPAGE_PCT": "2.0"}
        ):
            flags_obj = FeatureFlags()
