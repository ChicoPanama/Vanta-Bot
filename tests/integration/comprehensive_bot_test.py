...
class ComprehensiveBotTester:
        ...
        try:
            with patch("src.database.operations.db") as mock_db:
                ...
                if update.message.reply_text.called:
                    ...
                    message_text = args[0] if args else ""
                    if (
                        "market" in message_text.lower()
                        or "asset" in message_text.lower()
                    ):
        ...
        try:
            with patch("src.database.operations.db") as mock_db:
                ...
                try:
                    ...
                    try:
                        from src.bot.handlers.ai_insights_handlers import (
                            alfa_leaderboard,
                        )
                ...
                if update.message.reply_text.called:
                    ...
                    message_text = args[0] if args else ""
                    if (
                        "leaderboard" in message_text.lower()
                        or "alfa" in message_text.lower()
                    ):
            ...
            with patch("src.database.operations.db") as mock_db:
                ...
                try:
                    from src.bot.handlers.admin_commands import (
                        cmd_system_status as status_handler,
                    )
                ...
                if update.message.reply_text.called:
                    ...
                    message_text = args[0] if args else ""
                    if (
                        "status" in message_text.lower()
                        or "system" in message_text.lower()
                    ):
    ...
    async def test_10_1_invalid_command(self) -> str:
        ...
        try:
            ...
            self._create_mock_update("/invalidcommand")
    ...
    async def generate_report(self):
        ...
        for section_id, section_name in sections.items():
            section_tests = [
                r for r in self.results if r.test_id.startswith(section_id + ".")
            ]
            if section_tests:
                section_passed = len([r for r in section_tests if r.status == "PASS"])
                section_total = len(section_tests)
                section_rate = (
                    (section_passed / section_total) * 100 if section_total > 0 else 0
                )
                print(
                    f"  {section_id}. {section_name}: {section_passed}/{section_total} ({section_rate:.1f}%)"
                )
        ...
        for result in self.results:
            if result.status == "FAIL":
                if (
                    "import" in result.error_message.lower()
                    or "module" in result.error_message.lower()
                ):
                    critical_issues.append(
                        f"{result.test_id}: Import/module issue - {result.error_message}"
                    )
                elif (
                    "async" in result.error_message.lower()
                    or "await" in result.error_message.lower()
                ):
                    critical_issues.append(
                        f"{result.test_id}: Async/await issue - {result.error_message}"
                    )
        ...
        if passed == total_tests:
            ...
            print(
                "  ✅ Most tests passed. Address the failing tests for full functionality."
            )
        elif passed > total_tests * 0.5:
            print("  ⚠️ Some tests passed. Review and fix failing handlers.")
        else:
            print(
                "  ❌ Many tests failed. Check imports, dependencies, and handler implementations."
            )
                ...
                section_name: {
                    "total": len(
                        [
                            ...
                            if r.test_id.startswith(section_id + ".")
                        ]
                    ),
                    "passed": len(
                        [
