...
async def test_real_trading():
    ...
    try:
        ...
        from src.services.oracle import OracleFacade, MockOracleSource
        ...
        wallet_address = "0xC0A2470ABD1075a73d3176E2b4664C3c29bBA699"  # Fund this address
        
        print(f"✅ Using wallet: {wallet_address}")
        print(f"   (Fund this address with Base Sepolia ETH)")
        ...
        if eth_balance == 0:
            ...
            print(f"   Fund this wallet with Base Sepolia ETH:")
        ...
        try:
            ...
            receipt = base_client.w3.eth.wait_for_transaction_receipt(tx_result, timeout=120)
            
            if receipt.status == 1:
                print(f"✅ Transaction confirmed in block {receipt.blockNumber}")
                print(f"   Gas used: {receipt.gasUsed}")
                print(f"   Gas price: {base_client.w3.from_wei(receipt.effectiveGasPrice, 'gwei')} gwei")
            else:
        ...
        try:
            if positions:
                ...
                close_receipt = base_client.w3.eth.wait_for_transaction_receipt(close_result, timeout=120)
                
                if close_receipt.status == 1:
                    print(f"✅ Close transaction confirmed in block {close_receipt.blockNumber}")
                else:
