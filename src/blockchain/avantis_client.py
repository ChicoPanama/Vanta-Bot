...
class AvantisClient:
    ...
    def __init__(self, oracle=None, validator=None):
        ...
        if settings.AVANTIS_TRADING_CONTRACT:
            try:
                checksum = self.base_client.w3.to_checksum_address(
                    settings.AVANTIS_TRADING_CONTRACT
                )
            except Exception:
                checksum = settings.AVANTIS_TRADING_CONTRACT
            self.trading_contract = self.base_client.w3.eth.contract(
                address=checksum, abi=self.trading_abi
            )
        else:
        ...
        if settings.AVANTIS_VAULT_CONTRACT:
            try:
                v_checksum = self.base_client.w3.to_checksum_address(
                    settings.AVANTIS_VAULT_CONTRACT
                )
            except Exception:
                v_checksum = settings.AVANTIS_VAULT_CONTRACT
            self.vault_contract = self.base_client.w3.eth.contract(
                address=v_checksum, abi=self.vault_abi
            )
        else:
    ...
    async def open_position(
        ...
        try:
            ...
            if leverage <= 0 or leverage > settings.MAX_LEVERAGE:
                raise ValueError(
                    f"Invalid leverage: {leverage}. Must be 0 < leverage <= {settings.MAX_LEVERAGE}"
                )
            ...
            if is_ob_price:
                ...
                try:
                    dev_bps = int(
                        (Decimal(raw_quote.conf) * Decimal(10000))
                        / Decimal(max(raw_quote.price, 1))
                    )
                except Exception:
                    dev_bps = 0
            else:
            ...
            try:
                ...
                logger.warning(
                    f"SDK execution path failed, falling back to ABI encoding: {sdk_err}"
                )
                ...
                tx_hash = await self.base_client.submit(
                    tx_params=tx_params, request_id=request_id
                )

                logger.info(
                    f"Position opened (fallback): {tx_hash} for {market} {side} {size} @ {leverage}x"
                )
                return tx_hash
    ...
    def _calculate_min_out(self, price: Decimal, side: str) -> int:
        ...
        if side == "long":
            ...
            min_price = price * (
                Decimal("1") + Decimal(slippage_bps) / Decimal("10000")
            )
        else:
    ...
    async def close_position(
        self, wallet_id: str, position_id: int, request_id: str = None
    ) -> str:
        ...
        try:
            ...
            data = self.trading_contract.encodeABI(
                fn_name="closePosition", args=[position_id]
            )
    ...
    def get_position_info(self, position_id: int):
        ...
        return {"id": position_id, "status": "OPEN", "pnl": 0.0}
    ...
    async def set_take_profit_stop_loss(
        ...
        try:
            ...
            tx_hash = await self.base_client.submit(
                tx_params, request_id=f"tp_sl_{position_id}"
            )
            return tx_hash
    ...
    async def update_position_leverage(
        self, user_address: str, private_key: str, position_id: int, new_leverage: int
    ):
        ...
        try:
            ...
            tx_hash = await self.base_client.submit(
                tx_params, request_id=f"leverage_{position_id}"
            )
            return tx_hash
    ...
    async def partial_close_position(
        ...
        try:
            ...
            tx_hash = await self.base_client.submit(
                tx_params, request_id=f"partial_close_{position_id}"
            )
            return tx_hash
