...
from src.security.key_vault import WalletEncryption, LocalFernetKeyVault
import logging
...
class WalletManager:
    ...
    def create_wallet(self, user_id: str) -> dict:
        ...
        if self._wallet_encryption:
            ...
            wrapped_dek, encrypted_privkey = self._wallet_encryption.create_encrypted_wallet(
                wallet['private_key']
            )
            return {
            ...
            # Legacy encryption
            encrypted_private_key = self._encrypt_private_key_legacy(wallet['private_key'])
            return {
                'address': wallet['address'],
                'private_key': wallet['private_key'],  # Only for immediate use
    ...
    def get_private_key(self, wallet_data: dict) -> str:
        ...
        if wallet_data.get('encryption_version') == 'v2' and self._wallet_encryption:
            return self._wallet_encryption.decrypt_wallet(
                wallet_data['wrapped_dek'],
                wallet_data['encrypted_privkey']
            )
        elif wallet_data.get('encryption_version') == 'v1' and self._legacy_cipher_suite:
            return self._decrypt_private_key_legacy(wallet_data['encrypted_private_key'])
        else:
    ...
    def _decrypt_private_key_legacy(self, encrypted_private_key: str) -> str:
        ...
        return self._legacy_cipher_suite.decrypt(encrypted_private_key.encode()).decode()
