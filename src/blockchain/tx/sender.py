...
class TxSender:
    ...
    def send_with_retry(
        self, raw_tx: bytes, request_id: str, retries: int = 3, base_sleep: float = 0.5
    ) -> str:
        ...
        if existing_tx:
            logger.info(
                f"Transaction already sent for request {request_id}: {existing_tx.tx_hash}"
            )
    ...
    def bump_and_retry(
        self, original_tx: dict[str, Any], request_id: str, gas_policy
    ) -> str:
