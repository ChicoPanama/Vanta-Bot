...
class HybridNonceStore:
    ...
    def __init__(self, redis_client, web3_client):
        self.redis = redis_client
        self.web3 = web3_client
        self.redis_store = RedisNonceStore(redis_client, web3_client) if redis_client else None
    ...
    async def reserve(self, address: str):
        ...
        if self._redis_healthy and self.redis_store:
            try:
                ...
                logger.warning(f"Redis nonce store failed: {e}, falling back to in-memory")
                self._redis_healthy = False
