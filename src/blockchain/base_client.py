...
class BaseClient:
    ...
    def get_usdc_balance(self, address: str) -> float:
        ...
        try:
            ...
            balance_raw = contract.functions.balanceOf(self.w3.to_checksum_address(address)).call()
            # USDC has 6 decimals
    ...
    async def submit(self, tx_params: dict, *, request_id: str) -> str:
        ...
        try:
            ...
            from src.database.transaction_repo import TransactionRepository, calculate_payload_hash
            ...
            async with self.db_manager.get_session() as session:
                ...
                if existing:
                    logger.info(f"Transaction already exists: {request_id} -> {existing.tx_hash}")
    ...
    def send_transaction(self, 
        ...
        """
        logger.warning("send_transaction is deprecated - use submit() with injected signer")
        
        if not self.signer:
            raise NotImplementedError("No signer configured - use submit() method with injected signer")
        ...
        def run_in_thread():
            ...
            try:
                return loop.run_until_complete(self.submit(tx_params, request_id=request_id))
            finally:
                loop.close()
