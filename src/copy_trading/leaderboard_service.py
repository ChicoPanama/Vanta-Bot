...
class LeaderboardService:
    ...
    async def get_top_traders(self, limit: int = 50, window: str = "30d") -> list[dict]:
        ...
        try:
            ...
            await self.redis.setex(
                cache_key, self.cache_ttl, json.dumps(top_traders, default=str)
            )
    ...
    async def _fetch_filtered_traders(self, window: str) -> list[dict]:
        ...
        try:
            query = f"""
                ...
                WHERE ts.window = $1
                  AND ts.last_trade_at > NOW() - INTERVAL '{self.config.LEADER_ACTIVE_HOURS} hours'
        ...
        try:
            ...
            for trader in traders:
                ...
                volume_score = self._calculate_volume_score(
                    trader["last_30d_volume_usd"]
                )
                pnl_score = self._calculate_pnl_score(
                    trader["realized_pnl_clean_usd"], trader["last_30d_volume_usd"]
                )
            ...
            ranked_traders = sorted(
                traders, key=lambda x: x["ranking_score"], reverse=True
            )
        ...
        try:
            ...
            card_data = {
                ...
                **(ai_analysis or {}),
                ...
                "copyability_score": self._calculate_copyability_score(
                    {**stats, **(ai_analysis or {})}
                ),
    ...
    def _identify_strengths(
        self, stats: dict, ai_analysis: Optional[dict]
    ) -> list[str]:
    ...
    async def get_leaderboard_by_category(
        ...
        try:
            ...
            if category == "volume":
                ...
                return sorted(
                    all_traders,
                    key=lambda x: x.get("last_30d_volume_usd", 0),
                    reverse=True,
                )[:limit]
                ...
                return sorted(
                    all_traders,
                    key=lambda x: x.get("realized_pnl_clean_usd", 0),
                    reverse=True,
                )[:limit]
                ...
                return sorted(
                    all_traders,
                    key=lambda x: x.get("copyability_score", 50),
                    reverse=True,
                )[:limit]
    ...
    async def search_traders(self, query: str, limit: int = 10) -> list[dict]:
        ...
        try:
            ...
            for row in rows:
                ...
                trader_data["copyability_score"] = self._calculate_copyability_score(
                    trader_data
                )
    ...
    async def get_trader_analytics_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            acq = await self.db_pool.acquire()
            async with acq as conn:
                total_traders = await conn.fetchval(
                    "SELECT COUNT(*) FROM trader_stats WHERE window = '30d'"
                )
