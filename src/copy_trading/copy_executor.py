...
class CopyExecutor:
    ...
    def __init__(
        ...
        if self.avantis is None:
            raise ValueError(
                "Avantis client must be provided for live copy trading execution"
            )
            ...
            async with acq as conn:
                ...
                for row in rows:
                    config = CopyConfiguration(
                        ...
                        notional_cap=float(row["notional_cap"])
                        if row["notional_cap"]
                        else None,
        ...
        try:
            ...
            for leader_address in leaders:
                ...
                new_trades = await self._get_new_leader_trades(
                    leader_address, since=last_check_time
                )

                for trade in new_trades:
                    # Check if we should copy this trade
                    if await self._should_copy_trade(config, trade):
                        copy_request = await self._create_copy_request(
                            config, trade, leader_address
                        )
    ...
    async def _get_new_leader_trades(
        self, leader_address: str, since: datetime
    ) -> list[dict]:
    ...
    async def _should_copy_trade(self, config: CopyConfiguration, trade: dict) -> bool:
        ...
        try:
            ...
            if trade.get("leverage", 1) > config.max_leverage:
                logger.info(
                    f"Skipping copy due to leverage limit: {trade.get('leverage', 1)} > {config.max_leverage}"
                )
                return False
            ...
            try:
                if config.sizing_mode == "FIXED_NOTIONAL":
                    ...
                    target_notional = user_balance * (
                        float(config.sizing_value) / 100.0
                    )
                else:
                    logger.info(
                        f"Unknown sizing mode {config.sizing_mode}; skipping copy"
                    )
                    return False
            ...
            if not await self._check_rate_limit(config.copytrader_id, trade["pair"]):
                logger.info(
                    f"Skipping copy due to rate limit for copytrader {config.copytrader_id}"
                )
                return False
    ...
    async def _create_copy_request(
        self, config: CopyConfiguration, trade: dict, leader_address: str
    ) -> CopyTradeRequest:
    ...
    async def _get_user_balance(self, user_id: int) -> float:
        ...
        try:
            ...
            logger.warning(
                "Failed to aggregate open positions for user %s: %s", user_id, exc
            )
    ...
    async def _execute_copy_trade(self, request: CopyTradeRequest):
        ...
        try:
            ...
            price_impact = (
                abs(current_price - request.trade_data["price"])
                / request.trade_data["price"]
            )

            if price_impact * 10000 > request.max_slippage_bps:
                logger.warning(
                    f"Skipping copy trade due to slippage: {price_impact:.2%}"
                )
            ...
            tx_hash = await self._execute_avantis_trade(
                ...
                leverage=min(
                    request.trade_data["leverage"],
                    await self._get_max_leverage(request.copytrader_id),
                ),
    ...
    async def _get_current_price(self, pair: str) -> float:
        ...
        try:
            symbol = (
                pair if "/" in pair or "-" in pair else self._get_symbol_from_pair(pair)
            )
    ...
    async def _execute_avantis_trade(
        ...
        try:
            ...
            if (
                not credentials
                or not credentials["wallet_address"]
                or not credentials["encrypted_private_key"]
            ):
                raise ValueError(
                    "User wallet credentials are not configured for copy trading"
                )
            ...
            if order_type != "market":
                logger.warning(
                    "Limit/advanced orders not yet supported in copy executor; executing as market trade"
                )
            ...
            logger.info(
                "Executed Avantis trade",
                extra={"tx_hash": tx_hash, "user_id": user_id, "pair": symbol},
            )
            return tx_hash
    ...
    async def _send_copy_notification(
        self,
        ...
        try:
            ...
            if status == "success":
                logger.info(
                    f"Copy trade success notification for user {request.copytrader_id}: {tx_hash}"
                )
            else:
                logger.warning(
                    f"Copy trade failure notification for user {request.copytrader_id}: {error}"
                )
        ...
        try:
            ...
            async with acq as conn:
                ...
                for row in rows:
                    ...
                    is_still_open = await self._check_position_status(
                        row["our_tx_hash"]
                    )
    ...
    async def follow_trader(
        self, user_id: int, leader_address: str, config: dict
    ) -> dict:
    ...
    async def get_copy_status(self, user_id: int) -> dict:
        ...
        try:
            ...
            total_volume = sum(
                float(
                    (p.get("executed_size") or p.get("size") or 0)
                    * (p.get("executed_price") or p.get("entry_price") or 0)
                )
                for p in closed_dicts
            )
            open_positions_dicts = [
                dict(p) for p in recent_positions if p["status"] == "OPEN"
            ]
            total_volume += sum(
            ...
            return {
                ...
                "performance": {
                    ...
                    "copy_attribution": copy_pnl / (manual_pnl + copy_pnl)
                    if (manual_pnl + copy_pnl) != 0
                    else 0,
    ...
    async def _validate_copy_config(self, config: dict) -> dict:
        ...
        try:
            ...
            if (
                config["sizing_mode"] == "FIXED_NOTIONAL"
                and config["sizing_value"] <= 0
            ):
                return {"valid": False, "error": "Fixed notional must be positive"}

            # For safety and tests, cap PCT_EQUITY to 25%
            if config["sizing_mode"] == "PCT_EQUITY" and (
                config["sizing_value"] <= 0 or config["sizing_value"] > 25
            ):
        ...
        try:
            ...
            async with acq as conn:
                ...
                if existing:
                    ...
                    """,
                        user_id,
                        config.get("name", "Default Copytrader"),
                    )
