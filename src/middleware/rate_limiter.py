...
class RateLimitError(Exception):
    def __init__(self, limit_type: str, retry_after: int):
        ...
        super().__init__(
            f"Rate limit exceeded for {limit_type}. Retry after {retry_after}s"
        )
...
class RateLimiter:
    ...
    async def check_trading_limits(self, user_id: str) -> dict[str, Any]:
        ...
        for limit_type, config in limits.items():
            passed = await self.check_limit(
                user_id, limit_type, config["limit"], config["window"]
            )
    ...
    async def get_user_stats(self, user_id: str) -> dict[str, Any]:
        ...
        # Get current counts
        hourly_trades = await redis.get(
            f"rate_limit:open_positions:{user_id}:{now // 60}"
        )
        daily_trades = await redis.get(
            f"rate_limit:daily_trades:{user_id}:{now // 86400}"
        )
    ...
    async def check_telegram_limits(self, user_id: str) -> bool:
        ...
        return await self.check_limit(user_id, "telegram_messages", limit, window)
    ...
    async def check_copy_trading_limits(self, user_id: str) -> bool:
        ...
        return await self.check_limit(user_id, "copy_executions", limit, window)
        ...
        try:
            ...
            return {
                ...
                "limits": {
                    ...
                    "daily_trades": {"current": stats["daily_trades"], "limit": 50},
                    "hourly_volume": {
            ...
            return {"user_id": user_id, "error": str(e), "timestamp": time.time()}
