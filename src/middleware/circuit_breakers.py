...
class SimpleBreaker:
    ...
    def call(self, fn: Callable, *args, **kwargs) -> Any:
        ...
        if now < self.open_until:
            ...
            raise RuntimeError(
                f"Circuit breaker {self.name} is open until {self.open_until}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Circuit breaker {self.name} failure {self.fail_count}/{self.fail_threshold}: {e}"
            )

            if self.fail_count >= self.fail_threshold:
                self.open_until = now + self.reset_after
                logger.error(
                    f"Circuit breaker {self.name} OPEN until {self.open_until}"
                )
    ...
    def get_status(self) -> dict:
        ...
        return {
            name: {
                ...
                "time_until_reset": max(0, breaker.open_until - now)
                if breaker.is_open()
                else 0,
            }
