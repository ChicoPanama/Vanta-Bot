...
class SDKShim:
    ...
    def __init__(self, use_sdk: bool = False):
        ...
        if use_sdk:
            try:
                ...
                logger.error("Avantis SDK not available - falling back to direct contract calls")
                self.use_sdk = False
    ...
    def _validate_human_units(
        ...
        if collateral_usdc > 1000000:  # $1M seems like a reasonable max
            raise ValueError(f"Collateral {collateral_usdc} appears pre-scaled (too large)")
