...
from .math import TradeUnits, to_trade_units
from .models import HumanTradeParams, RiskLimits, TradeInput, TradeLimits
...
def validate_trade_input(trade: TradeInput, limits: TradeLimits) -> list[str]:
    ...
    if trade["initialPosToken"] <= 0:
        ...
        errors.append(
            f"Position size {trade['initialPosToken']} below minimum {limits['minPositionSize']}"
        )
    elif trade["initialPosToken"] > limits["maxPositionSize"]:
        errors.append(
            f"Position size {trade['initialPosToken']} above maximum {limits['maxPositionSize']}"
        )
    ...
    if trade["leverage"] <= 0:
        ...
        errors.append(
            f"Leverage {trade['leverage']} above maximum {limits['maxLeverage']}"
        )
    ...
    if trade["pairIndex"] < 0:
        ...
        errors.append(
            f"Pair index {trade['pairIndex']} exceeds maximum {limits['maxPairs'] - 1}"
        )
...
def validate_risk_limits(
    params: HumanTradeParams, risk_limits: RiskLimits, current_balance: Decimal
) -> list[str]:
    """
...
def clamp_trade_params(
    ...
    collateral_clamped = max(
        collateral_min, min(params.collateral_usdc, collateral_max)
    )
...
def validate_scaling_consistency(trade_units: TradeUnits) -> bool:
    ...
    if trade_units.leverage > 10**15:
        logger.error(
            f"Suspected double-scaling: leverage {trade_units.leverage} > 10^15"
        )
        return False
    ...
    if trade_units.slippage > 10**15:
        logger.error(
            f"Suspected double-scaling: slippage {trade_units.slippage} > 10^15"
        )
        return False

    # Check position size consistency
    expected_position_size = (trade_units.initial_pos_token * trade_units.leverage) // (
        10**10
    )
...
def validate_market_order_invariant(trade: TradeInput) -> bool:
    ...
    if trade["openPrice"] != 0:
        logger.error(
            f"Market order invariant violation: openPrice {trade['openPrice']} != 0"
        )
        return False
