...
def to_trade_units(
    collateral_usdc: Decimal, leverage_x: Decimal, slippage_pct: Decimal
) -> TradeUnits:
    """
...
def validate_scaling_invariant(trade_units: TradeUnits) -> bool:
    ...
    expected_position_size = (
        trade_units.initial_pos_token * trade_units.leverage
    ) // int(SCALE_1E10)
    return trade_units.position_size_usdc == expected_position_size


def get_max_safe_leverage(
    collateral_usdc: Decimal, max_position_usdc: Decimal
) -> Decimal:
    """
...
def calculate_liquidation_price(
    entry_price: Decimal, leverage: Decimal, is_long: bool
) -> Decimal:
    """
