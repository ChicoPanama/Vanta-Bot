...
class AvantisPriceProvider:
    ...
    async def get_loss_protection(self, trade_input: TradeInput, opening_fee_usdc: float) -> LossProtectionInfo:
        """
    ...
    async def get_price_impact_spread(self, pair: str, is_long: bool, position_size: float) -> Optional[float]:
        """
    ...
    async def get_skew_impact_spread(self, pair: str, is_long: bool, position_size: float) -> Optional[float]:
        """
    ...
    async def quote_open(self, pair: str, is_long: bool, collateral_usdc: float, 
        ...
        try:
            ...
            # Get loss protection
            loss_protection = await self.get_loss_protection(trade_input, opening_fee_usdc)
    ...
    async def get_pair_info(self, pair: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            price_impact_long = await self.get_price_impact_spread(pair, True, small_position_size)
            price_impact_short = await self.get_price_impact_spread(pair, False, small_position_size)
            skew_impact_long = await self.get_skew_impact_spread(pair, True, small_position_size)
            skew_impact_short = await self.get_skew_impact_spread(pair, False, small_position_size)
