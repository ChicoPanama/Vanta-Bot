...
class AvantisExecutor:
    ...
    def _build_trade_input(
        self, order: OrderRequest, trader_address: str, pair_index: int = 0
    ) -> "AvantisExecutor._TradeInputCompat":
        """
    ...
    def _get_order_type(
        self, order: OrderRequest, zero_fee: bool = False
    ) -> TradeInputOrderType:
        """
        ...
        if order.order_type.lower() == "market":
            return (
                ...
                if zero_fee
                else TradeInputOrderType.MARKET
            )
        elif order.order_type.lower() == "limit":
            return TradeInputOrderType.LIMIT
        else:
            logger.warning(
                f"Unknown order type: {order.order_type}, defaulting to MARKET"
            )
            return (
                TradeInputOrderType.MARKET_ZERO_FEE
                if zero_fee
    ...
    async def open_market(
        ...
        try:
            ...
            # Structured logging
            logger.info(
                f"🎯 Trade Request: {order.pair} {'LONG' if order.is_long else 'SHORT'} "
            ...
            sdk_client = get_sdk_client()
            allowance_ok = await sdk_client.ensure_usdc_allowance(
                order.collateral_usdc, trader_address
            )
            if not allowance_ok:
                return TradeResult(
                    success=False, error="Failed to ensure USDC allowance"
                )

            # Build and execute trade
            order_type = self._get_order_type(order, zero_fee)
            sdk_input = (
                trade_input.to_sdk() if hasattr(trade_input, "to_sdk") else trade_input
            )
            tx = await client.trade.build_trade_open_tx(
                sdk_input, order_type, order.slippage_pct
            )
            ...
            if tx_hash:
                logger.info(
                    f"✅ LIVE Trade executed: {order.pair} {'LONG' if order.is_long else 'SHORT'} "
                ...
                return TradeResult(
                    success=False, error="Transaction failed to broadcast"
                )
    ...
    async def close_trade(
        ...
        try:
            ...
            if tx_hash:
                logger.info(
                    f"✅ Trade closed: pair {pair_index}, trade {trade_index}, "
                ...
                return TradeResult(
                    success=False, error="Close transaction failed to broadcast"
                )
    ...
    async def estimate_gas(self, order: OrderRequest) -> Optional[int]:
        ...
        try:
            ...
            pair_index = await (await self._get_price_provider()).get_pair_index(
                order.pair
            )
