...
class LeverageSafetyManager:
    ...
    def _load_limits_from_config(self) -> RiskLimits:
        ...
        return RiskLimits(
            max_position_size_usd=Decimal(
                str(os.getenv("MAX_POSITION_SIZE_USD", "100000"))
            ),
            max_account_risk_pct=Decimal(
                str(os.getenv("MAX_ACCOUNT_RISK_PCT", "0.10"))
            ),
            max_leverage=Decimal(str(os.getenv("MAX_LEVERAGE", "500"))),
            liquidation_buffer_pct=Decimal(
                str(os.getenv("LIQUIDATION_BUFFER_PCT", "0.05"))
            ),
    ...
    def validate_position(
        ...
        if position_size_usd > self.limits.max_position_size_usd:
            raise RiskError(
                f"Position size ${position_size_usd} exceeds limit ${self.limits.max_position_size_usd}"
            )
        ...
        if margin_required > account_balance:
            raise RiskError(
                f"Insufficient margin: need ${margin_required}, have ${account_balance}"
            )
        ...
        liquidation_price_distance = (
            Decimal("1") / leverage
        )  # 1/leverage = liquidation distance
        ...
        return {
            ...
            "risk_score": min(
                1.0, float(account_risk_pct / self.limits.max_account_risk_pct)
            ),
        }

    def calculate_max_position_size(
        self, account_balance: Decimal, leverage: Decimal
    ) -> Decimal:
    ...
    def calculate_safe_leverage(
        self, account_balance: Decimal, position_size_usd: Decimal
    ) -> Decimal:
    ...
    def get_risk_summary(self, account_balance: Decimal) -> dict[str, Any]:
        ...
        return {
            ...
            "max_safe_position_10x": float(
                self.calculate_max_position_size(account_balance, Decimal("10"))
            ),
            "max_safe_position_100x": float(
                self.calculate_max_position_size(account_balance, Decimal("100"))
            ),
            "max_safe_position_500x": float(
                self.calculate_max_position_size(account_balance, Decimal("500"))
            ),
        }
