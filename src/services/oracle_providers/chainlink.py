...
class ChainlinkOracle(Oracle):
    ...
    def __init__(self, web3: Web3, validate_on_init: bool = True):
        ...
        if feeds_config.is_available():
            ...
            logger.info(
                f"Loaded Chainlink feeds from centralized config: {len(self.aggregators)} feeds"
            )
        else:
            ...
            self.aggregators = {
                ...
                "EURC": os.getenv(
                    "CHAINLINK_EURC_USD_FEED",
                    "0xDAe398520e2B67cd3f27aeF9Cf14D93D927f8250",
                ),
            }
            logger.info(
                f"Using environment variables for Chainlink feeds: {len(self.aggregators)} feeds"
            )
    ...
    def _validate_feeds_on_startup(self):
        ...
        for symbol, address in self.aggregators.items():
            try:
                ...
                contract = self.web3.eth.contract(
                    address=self.web3.to_checksum_address(address), abi=aggregator_abi
                )
                ...
                if answered_in_round != round_id:
                    raise ValueError(
                        f"Feed {symbol} has stale round data: {answered_in_round} != {round_id}"
                    )
                ...
                if symbol in price_ranges:
                    ...
                    if not (min_price <= price_usd <= max_price):
                        raise ValueError(
                            f"Feed {symbol} price ${price_usd:,.2f} outside expected range ${min_price:,} - ${max_price:,}"
                        )
                ...
                if age_seconds > max_age:
                    if os.getenv("ENVIRONMENT", "development") == "production":
                        ...
                        logger.warning(
                            f"⚠️  {symbol} feed is stale: {age_seconds}s old (development mode - continuing)"
                        )

                logger.info(
                    f"✅ {symbol} feed validated: ${price_usd:,.2f} (age: {age_seconds}s)"
                )

            except Exception as e:
                logger.error(f"❌ Chainlink feed validation failed for {symbol}: {e}")
                raise RuntimeError(
                    f"CRITICAL: Chainlink feed validation failed for {symbol} at {address}: {e}. "
