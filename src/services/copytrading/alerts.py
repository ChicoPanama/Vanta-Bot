...
async def fanout_trader_signal(bot: Bot, trader_key: str, signal: Dict[str, Any]):
  ...
  log.info("fanout_complete", extra={
    "trader_key": trader_key,
    "pair": signal.get("pair"),
    "side": signal.get("side"),
    "user_count": len(following_users),
...
async def on_trader_signal(bot: Bot, uid: int, trader_key: str, signal: Dict[str, Any]):
    ...
    for attempt in range(3):
      try:
        ...
        if attempt == 2:  # Last attempt
          log.exception("signal_notify_fail after 3 attempts", extra={
            "user_id": uid,
            "trader_key": trader_key,
            "attempt": attempt + 1,
            "error": str(e)
  ...
  log.info("autocopy_decision", extra={
    "user_id": uid,
    "trader_key": trader_key,
    "pair": signal.get("pair"),
    "side": signal.get("side"),
  ...
  if not ok and why in ["server_dry", "executor_missing"]:
    ...
    if not _is_notification_deduplicated(notification_key):
      ...
      for attempt in range(3):
        try:
          if why == "server_dry":
            await bot.send_message(chat_id=uid, text="ℹ️ Auto-copy is ON but server is in DRY mode.\nTrades will not be executed until server switches to LIVE mode.")
          elif why == "executor_missing":
            await bot.send_message(chat_id=uid, text="ℹ️ Auto-copy set to ON, but executor is not installed on server.\nTrades are not mirrored yet.")
          break  # Success, exit retry loop
        except Exception as e:
          if attempt == 2:  # Last attempt
            log.warning("notification_dedupe_fail", extra={
              "user_id": uid,
              "reason": why,
              "attempt": attempt + 1,
              "error": str(e)
