...
async def maybe_autocopy_on_signal(
    ...
    if cfg.get("per_trade_cap_usd") and notional > Decimal(
        str(cfg["per_trade_cap_usd"])
    ):
    ...
    if sizing == "MIRROR":
        ...
        collateral_usdc = Decimal(str(cfg.get("fixed_usd", 100.0))) / (
            lev if lev > 0 else Decimal("1")
        )
    else:  # PCT_EQUITY
    ...
    try:
        ...
        log.exception(
            "copy_exec_fail", extra={"user_id": uid, "trader_key": trader_key}
        )
...
async def executor_status(user_id: int, trader_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        log.warning(
            ...
            extra={"user_id": user_id, "trader_key": trader_key, "error": str(exc)},
        )
...
async def get_status(uid: int) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "follows": [{"trader_key": tk, "config": cfg} for tk, cfg in follows],
        "aggregate": None,
    }
    ...
    async def _leader_status(trader_key: str) -> dict[str, Any]:
        ...
        return (
            data
            if data.get("ok")
            else {"ok": False, "error": data.get("error", "unknown")}
        )

    leader_tasks = [asyncio.create_task(_leader_status(tk)) for tk, _ in follows]
