...
class MockOracleSource(OracleSource):
    ...
    def get_price(self, market: str) -> PriceQuote:
        ...
        return PriceQuote(
            price=price, timestamp=int(time.time()), source=self.name, freshness_sec=0
        )
        ...
        try:
            ...
            for source in self.sources:
                try:
                    ...
                    logger.warning(
                        f"Source {source.__class__.__name__} failed for {market}: {e}"
                    )
                    continue
            ...
            now = int(time.time())
            fresh_quotes = [
                q for q in quotes if (now - q.timestamp) <= self.max_freshness_sec
            ]

            if not fresh_quotes:
                raise ValueError(
                    f"No fresh prices for {market} (max age: {self.max_freshness_sec}s)"
                )
            ...
            if deviation_bps > self.max_deviation_bps:
                raise ValueError(
                    f"Price deviation too high: {deviation_bps}bps > {self.max_deviation_bps}bps"
                )
...
class PriceValidator:
    ...
    def validate_quote(self, quote: PriceQuote) -> bool:
        ...
        try:
            ...
            if quote.freshness_sec > self.max_freshness_sec:
                logger.warning(
                    f"Price too stale: {quote.freshness_sec}s > {self.max_freshness_sec}s"
                )
                return False
            ...
            if quote.deviation_bps > self.max_deviation_bps:
                logger.warning(
                    f"Price deviation too high: {quote.deviation_bps}bps > {self.max_deviation_bps}bps"
                )
                return False
    ...
    def validate_prices(self, quotes: dict[str, PriceQuote]) -> dict[str, bool]:
        ...
        """
        return {market: self.validate_quote(quote) for market, quote in quotes.items()}
        ...
        try:
            if self.secondary is not None:
                ...
                async def _run_primary():
                    ...
                    try:
                        res = _call_provider(
                            self.primary, canonical, max_age_s, max_dev_bps
                        )
                ...
                async def _run_secondary():
                    ...
                    try:
                        res = _call_provider(
                            self.secondary, canonical, max_age_s, max_dev_bps
                        )
...
def create_default_oracle() -> OracleFacade:
    ...
    from .oracle_providers import ChainlinkOracle, PythOracle

    # Check if we're in test mode
    if (
        os.getenv("ENVIRONMENT", "production") == "test"
        or os.getenv("PRICING_PROVIDER") == "mock"
    ):
    ...
    try:
        ...
        raise RuntimeError(
            f"Oracle initialization failed: {e}. Cannot start with random price fallback in production."
        )
...
def create_price_validator() -> PriceValidator:
    ...
    return PriceValidator(
        max_deviation_bps=max_deviation_bps, max_freshness_sec=max_freshness_sec
    )
