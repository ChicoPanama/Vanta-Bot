...
class AvantisRegistry:
    ...
    async def resolve_vault_address(self) -> Optional[str]:
        ...
        logger.info(
            f"🔍 Resolving vault address from Trading Proxy: {self.trading_contract_address}"
        )
        ...
        for abi in self.candidate_abis:
            try:
                ...
                contract = self.w3.eth.contract(
                    address=self.trading_contract_address, abi=[abi]
                )
                ...
                if (
                    ...
                    logger.info(
                        f"✅ Resolved vault address: {vault_address} via {function_name}()"
                    )
        ...
        logger.info(
            "Bot will continue without vault contract (trading features will work)"
        )
        return None
    ...
    async def get_contract_info(self) -> dict:
        ...
        return {
            ...
            else f"Chain {self.w3.eth.chain_id}",
        }
