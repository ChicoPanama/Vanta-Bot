...
class BackgroundServiceManager:
    ...
    async def _start_price_feed_client(self) -> None:
        ...
        try:
            ...
            if feed_client.is_configured():
                ...
                from src.services.markets.avantis_price_provider import (
                    get_price_provider,
                )
                ...
                async def price_callback(price_update):
                    logger.debug(
                        f"Price update: {price_update.pair} = {price_update.price}"
                    )
    ...
    async def _start_contract_registry(self) -> None:
        ...
        try:
            ...
            # Initialize contract registry
            initialize_registry(w3, settings.AVANTIS_TRADING_CONTRACT)
            ...
            try:
                ...
                if vault_address:
                    ...
                    logger.warning(
                        "⚠️ Could not resolve vault address from Trading Proxy"
                    )
            except Exception as e:
    ...
    async def _start_position_tracker(self) -> None:
        ...
        try:
            ...
            if db_url.startswith("postgresql+asyncpg://"):
                ...
                logger.info(
                    "Converted asyncpg URL to sync psycopg URL for position tracker"
                )
    ...
    async def _start_avantis_indexer(self) -> None:
        ...
        try:
            ...
            if db_url.startswith("postgresql+asyncpg://"):
                db_url = db_url.replace("postgresql+asyncpg://", "postgresql://", 1)
                logger.info(
                    "Converted asyncpg URL to sync psycopg URL for background services"
                )
            ...
            logger.info(
                f"🔄 Starting indexer backfill from block {backfill_from} to {latest}"
            )

            # Do initial backfill, then start tailing
            task = asyncio.create_task(
                self._run_indexer_with_backfill(indexer, backfill_from, latest)
            )
            self.services.append(task)
