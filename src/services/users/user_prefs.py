...
class UserPrefs:
    ...
    def get(self, user_id: int) -> Dict[str, Any]:
        ...
        con = self._conn()
        try:
            cur = con.execute("SELECT data FROM user_prefs WHERE user_id = ?", (user_id,))
            row = cur.fetchone()
            if not row:
    ...
    def put(self, user_id: int, prefs: Dict[str, Any]):
        ...
        try:
            ...
            con.execute("INSERT INTO user_prefs(user_id, data) VALUES(?, ?) ON CONFLICT(user_id) DO UPDATE SET data=excluded.data", (user_id, blob))
            con.commit()
        finally:
            con.close()

# Global accessor
_store: Optional[UserPrefs] = None
