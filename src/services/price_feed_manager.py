...
class PriceFeedManager:
    ...
    async def get_validated_price(self, asset: str) -> PriceData:
        ...
        try:
            results = await asyncio.wait_for(
                asyncio.gather(*price_tasks, return_exceptions=True), timeout=3.0
            )
        ...
        for i, result in enumerate(results):
            if isinstance(result, PriceData) and not result.is_stale:
                ...
                log.warning(
                    "Price source %s failed: %s", self.sources[asset][i].name, result
                )
    ...
    async def _fetch_price_safe(self, source: PriceSource, asset: str) -> PriceData:
        ...
        try:
            ...
            return PriceData(
                price=Decimal(str(price)), timestamp=time.time(), source=source.name
            )
    ...
    def _cross_validate_prices(self, prices: list[PriceData], asset: str) -> PriceData:
        ...
        weighted_sum = sum(p.price * getattr(p, "weight", 1.0) for p in prices)
    ...
    async def get_price_with_fallback(
        self, asset: str, fallback_price: Optional[Decimal] = None
    ) -> Decimal:
        ...
        try:
            ...
            if fallback_price:
                ...
                if asset in self.last_prices:
                    log.error(
                        "Using stale price for %s: %s (age: %.1fs)",
                        asset,
            ...
            try:
                ...
                if not coingecko_id:
                    raise PriceFeedError(
                        f"Unsupported asset for CoinGecko: {asset_name}"
                    )
