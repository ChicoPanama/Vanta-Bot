...
class RiskCalculator:
    ...
    def __init__(self, cfg: RiskEducationCfg | None = None):
    ...
    async def analyze(
        ...
        *,
        ...
        asset: str,
        protocol_max_leverage: int | None = 500,
    ) -> dict[str, Any]:
        # Technical validations (only impossibilities)
        if position_size_usd <= 0 or leverage <= 0 or account_balance_usd <= 0:
            raise ValueError(
                "Position size, leverage, and account balance must be positive."
            )

        margin_required = position_size_usd / leverage
        if margin_required >= account_balance_usd:
            raise ValueError(
                f"Insufficient balance: need ${margin_required:,.2f}, have ${account_balance_usd:,.2f}"
            )
        ...
        account_risk_pct = (
            stress["loss"] / account_balance_usd
        ...
        warnings = self._warnings(
            leverage, account_risk_pct, liq_distance_pct, protocol_max_leverage
        )
    ...
    def _scenarios(self, size: Decimal, equity: Decimal) -> dict[str, dict[str, Any]]:
        def mk(move: Decimal):
            ...
            impact = (
                (loss / equity).quantize(Decimal("0.0001")) if equity else Decimal("0")
            )
    ...
    def _warnings(
        ...
        protocol_max_leverage: int | None,
    ) -> list[dict[str, str]]:
        w: list[dict[str, str]] = []
    ...
    def _quality(
        self, lev: Decimal, acct_risk: Decimal, liq_d: Decimal
    ) -> dict[str, Any]:
        score = 100
        if lev > 100:
            score -= 30
        ...
        if score >= 80:
            rating, desc = (
                "Excellent",
                "Conservative risk profile with good safety margins.",
            )
        elif score >= 60:
