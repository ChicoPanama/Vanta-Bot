...
class RiskCalculator(object):
    ...
    def suggest_sizes(self, account_balance_usd: Decimal, leverage: Decimal) -> Dict[str, str]:
    ...
    def _quality(self, lev: Decimal, acct_risk: Decimal, liq_dist: Decimal) -> Dict[str, str]:
        score = 100
        if lev > Decimal('100'): score -= 30
        elif lev > Decimal('50'): score -= 15
        elif lev > Decimal('25'): score -= 5

        if acct_risk > Decimal('0.10'): score -= 25
        elif acct_risk > Decimal('0.05'): score -= 10

        if liq_dist < Decimal('0.01'): score -= 20
        elif liq_dist < Decimal('0.02'): score -= 10
