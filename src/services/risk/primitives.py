...
def calculate_pnl(
    entry_price: Decimal, current_price: Decimal, size: Decimal, side: str
) -> Decimal:
...
def calculate_margin_ratio(
    entry_price: Decimal, current_price: Decimal, leverage: Decimal, side: str
) -> Decimal:
...
def calculate_risk_score(position: PositionInfo) -> Decimal:
    ...
    try:
        ...
        calculate_pnl(
            position.entry_price, position.current_price, position.size, position.side
        )
        ...
        # Risk factors
        leverage_risk = min(
            position.leverage / Decimal("10"), Decimal("1")
        ...
        margin_risk = max(
            Decimal("0"),
            (Decimal("1") - margin_ratio)
            / (Decimal("1") - position.maintenance_margin),
        )
...
def calculate_portfolio_risk(positions: list[PositionInfo]) -> Decimal:
    ...
    try:
        ...
        concentration_factor = Decimal("1") - min(
            Decimal(len(positions)) / Decimal("10"), Decimal("0.5")
        )
