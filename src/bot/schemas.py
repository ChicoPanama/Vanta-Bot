...
class OpenPositionCommand(BaseModel):
    ...
    leverage: Decimal = Field(
        ..., gt=0, le=settings.MAX_LEVERAGE, description="Leverage multiplier"
    )
...
class PositionsCommand(BaseModel):
    ...
    status: Optional[Literal["open", "closed", "all"]] = Field(
        "open", description="Position status filter"
    )
    limit: int = Field(
        10, ge=1, le=50, description="Maximum number of positions to show"
    )
...
class AdminCommand(BaseModel):
    ...
    action: Literal["status", "users", "emergency_stop", "maintenance"] = Field(
...
class EmergencyStopCommand(BaseModel):
    ...
    reason: str = Field(
        ..., min_length=1, max_length=200, description="Reason for emergency stop"
    )
    scope: Literal["all", "trading", "copy_trading"] = Field(
        "all", description="Scope of emergency stop"
    )
...
class MaintenanceCommand(BaseModel):
    ...
    message: Optional[str] = Field(
        None, max_length=500, description="Maintenance message"
    )
...
class UserManagementCommand(BaseModel):
    ...
    action: Literal[
        "add_admin", "remove_admin", "add_super_admin", "remove_super_admin", "list"
    ] = Field(..., description="User management action")
...
class MetricsCommand(BaseModel):
    ...
    metric_type: Literal["system", "trading", "users", "all"] = Field(
        "all", description="Type of metrics to show"
    )
    time_range: Literal["1h", "24h", "7d", "30d"] = Field(
        "24h", description="Time range for metrics"
    )
    ...
    def validate_command(cls, v):
        if v is not None:
            ...
            if v not in valid_commands:
                raise ValueError(
                    f"Unknown command: {v}. Valid commands: {', '.join(valid_commands)}"
                )
        return v
