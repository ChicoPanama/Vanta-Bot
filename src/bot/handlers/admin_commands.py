...
from src.services.copy_trading.execution_mode import ExecutionMode, execution_manager
...
async def cmd_copy_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) != 1:
        await update.message.reply_text(
            ...
            f"Current mode: `{execution_manager.mode.value}`\n"
            f"Emergency stop: `{execution_manager.is_emergency_stopped}`",
            parse_mode="Markdown",
        )
        return
...
async def cmd_emergency_stop(
    ...
    if action == "stop":
        ...
        await update.message.reply_text(
            "❌ Invalid action. Use `stop` or `start`", parse_mode="Markdown"
        )
