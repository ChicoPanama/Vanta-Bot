...
async def follow_trader_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        trader_address = update.callback_query.data.split('_')[2]  # follow_trader_0x123...
...
async def my_copytraders_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.edit_message_text(
            text,
            parse_mode='Markdown',
            reply_markup=get_copytraders_keyboard()
        )
...
def get_copy_trading_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔧 Create Copy Trader", callback_data="create_copytrader")],
...
def get_leaderboard_keyboard():
    ...
    for i in range(1, 6):
        keyboard.append([
            InlineKeyboardButton(f"Follow Trader {i}", callback_data=f"follow_trader_0x{'a'*40}")
        ])
    
    keyboard.append([InlineKeyboardButton("🔄 Refresh", callback_data="alfa_leaderboard")])
