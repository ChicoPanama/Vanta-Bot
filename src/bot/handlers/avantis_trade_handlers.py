...
async def a_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if pair_info["pair_spread"]:
            ...
            response += (
                f"**Current Spread:** {spread.bid_price:.4f} / {spread.ask_price:.4f}\n"
            )

        if pair_info["price_impact_long"]:
            response += (
                f"**Price Impact (Long):** {pair_info['price_impact_long']:.2f}%\n"
            )

        if pair_info["price_impact_short"]:
            response += (
                f"**Price Impact (Short):** {pair_info['price_impact_short']:.2f}%\n"
            )

        if pair_info["skew_impact_long"]:
            response += (
                f"**Skew Impact (Long):** {pair_info['skew_impact_long']:.2f}%\n"
            )

        if pair_info["skew_impact_short"]:
            response += (
                f"**Skew Impact (Short):** {pair_info['skew_impact_short']:.2f}%\n"
            )
...
async def a_open_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if side not in ["long", "short"]:
            await update.message.reply_text(
                "❌ **Invalid side.** Use `long` or `short`.", parse_mode="Markdown"
            )
            return
        ...
        quote = await price_provider.quote_open(
            pair, side == "long", collateral, leverage
        )
        ...
        if result.success:
            ...
            response += (
                f"**Loss Protection:** `{quote['loss_protection_percent']:.2f}%`\n"
            )
            ...
            await update.message.reply_text(
                f"❌ **Failed to open position:** {result.error}", parse_mode="Markdown"
            )
...
async def a_trades_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Format response
        response = "📊 **Open Positions & Orders**\n\n"
        response += f"**Trader:** `{trader_address[:10]}...`\n\n"
        ...
        if trades:
            ...
            for i, trade in enumerate(trades[:5], 1):  # Show max 5 trades
                try:
                    ...
                    response += (
                        f"   Size: `{summary.size:.2f} USDC` @ `{summary.leverage}x`\n"
                    )
...
async def a_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if fraction == 1.0:
            ...
            result = await positions_manager.close_partial(
                trader_address, target_trade, fraction
            )
        ...
        if result.success:
            ...
            if result.closed_collateral:
                response += (
                    f"**Closed Collateral:** `{result.closed_collateral:.2f} USDC`\n"
                )
...
async def a_pairs_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        crypto_pairs = [
            p
            for p in pairs
            if any(crypto in p for crypto in ["BTC", "ETH", "SOL", "AVAX", "MATIC"])
        ]
...
async def a_execmode_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if success:
            ...
            await update.message.reply_text(
                "❌ **Invalid mode.** Use `DRY` or `LIVE`.", parse_mode="Markdown"
            )
