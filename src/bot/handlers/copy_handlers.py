...
def _cfg_text(tk: str, cfg: Dict[str, Any]) -> str:
  ...
  lines.append(f"Auto-copy: {'ON' if cfg.get('auto_copy') else 'OFF'}   Notify: {'ON' if cfg.get('notify') else 'OFF'}")
  ...
  lines.append(f"Per-trade cap: {fmt_usd(Decimal(str(cfg.get('per_trade_cap_usd', 0))))}")
  lines.append(f"Daily cap: {fmt_usd(Decimal(str(cfg.get('daily_cap_usd', 0))))}")
  lines.append(f"Loss protection: {'ON' if cfg.get('use_loss_protection') else 'OFF'}")
  lines.append(f"Stop after losses: {cfg.get('stop_after_losses')}")
  lines.append(f"Stop after drawdown: {fmt_usd(Decimal(str(cfg.get('stop_after_drawdown_usd', 0))))}")
  
  # Add active stop rule preview
  if cfg.get('use_loss_protection'):
    stop_rules = []
    if cfg.get('stop_after_losses', 0) > 0:
      stop_rules.append(f"⚠️ Loss streak stop @ {cfg.get('stop_after_losses')} losses")
    if cfg.get('stop_after_drawdown_usd', 0) > 0:
      stop_rules.append(f"⚠️ Drawdown stop @ {fmt_usd(Decimal(str(cfg.get('stop_after_drawdown_usd'))))}")
    if stop_rules:
...
def _cfg_rows(tk: str, cfg: Dict[str, Any]):
  ...
  rows.extend([
    [("Sizing: Mirror", f"cp:size:mirror:{tk}"), ("Fixed $", f"cp:size:fixed:{tk}"), ("% Equity", f"cp:size:pct:{tk}")],
    [("MaxLev +10", f"cp:lev:up:{tk}"), ("-10", f"cp:lev:down:{tk}")],
    [("Slip 0.5%", f"cp:slip:0.5:{tk}"), ("1%", f"cp:slip:1:{tk}"), ("2%", f"cp:slip:2:{tk}")],
    [("PerTrade +100", f"cp:cap_trade:+100:{tk}"), ("-100", f"cp:cap_trade:-100:{tk}")],
    [("DailyCap +1000", f"cp:cap_day:+1000:{tk}"), ("-1000", f"cp:cap_day:-1000:{tk}")],
    [("LossProt ⏻", f"cp:toggle:lossprot:{tk}"), ("StopLosses +1", f"cp:streak:+1:{tk}"), ("-1", f"cp:streak:-1:{tk}")],
...
async def cmd_follow(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if not args:
    await update.effective_chat.send_message("Usage: `/follow <trader_id_or_address>`", parse_mode="Markdown")
    return
  tk = args[0]
  ...
  await update.effective_chat.send_message(_cfg_text(tk, cfg), parse_mode="Markdown",
      reply_markup=kb(_cfg_rows(tk, cfg)))

async def cmd_unfollow(update: Update, context: ContextTypes.DEFAULT_TYPE):
  args = context.args or []
  if not args:
    await update.effective_chat.send_message("Usage: `/unfollow <trader_id_or_address>`", parse_mode="Markdown")
    return
  tk = args[0]
  copy_service.unfollow(update.effective_user.id, tk)
  await update.effective_chat.send_message(f"✅ Unfollowed `{tk}`", parse_mode="Markdown")
...
async def cmd_following(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if not follows:
    await update.effective_chat.send_message("You are not following any traders.\nUse `/follow <id>` or tap *Follow* in /alfa top50.",
                                             parse_mode="Markdown")
    return
  lines = ["*Following*"]
  rows = []
  for tk, cfg in follows:
    ...
    rows.append([("⚙️ Settings", f"cp:open:{tk}"), ("❌ Unfollow", f"cp:unfollow:{tk}")])
...
async def cb_copy(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if action == "open":
    ...
    await q.edit_message_text(_cfg_text(tk, cfg), parse_mode="Markdown", reply_markup=kb(_cfg_rows(tk, cfg)))
    return
  ...
  if action == "toggle":
    if kind == "auto": cfg["auto_copy"] = not cfg.get("auto_copy", False)
    elif kind == "notify": cfg["notify"] = not cfg.get("notify", True)
    elif kind == "lossprot": cfg["use_loss_protection"] = not cfg.get("use_loss_protection", True)

  elif action == "size":
    if kind == "mirror": cfg["sizing_mode"] = "MIRROR"
    elif kind == "fixed":
    ...
    cfg["slippage_override_pct"] = float(rest[0]) if kind in ("0.5","1","2") else float(kind)

  elif action == "cap_trade":
    delta = float(kind.replace("+","")) if "+" in kind else float(kind)
    cfg["per_trade_cap_usd"] = max(0.0, float(cfg.get("per_trade_cap_usd", 500.0) + delta))
    ...
    delta = float(kind.replace("+","")) if "+" in kind else float(kind)
    cfg["stop_after_drawdown_usd"] = max(0.0, float(cfg.get("stop_after_drawdown_usd", 1000.0) + delta))
