...
async def position_management_handler(
...
async def close_all_positions_handler(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
        ...
        for position in positions:
            try:
                ...
                tx_hash = avantis_client.close_position(
                    db_user.wallet_address, private_key, position.id
                )
...
async def close_profitable_positions_handler(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
...
async def close_losing_positions_handler(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
...
────────────────────
        ...
        await query.edit_message_text(
            history_text, parse_mode="Markdown", reply_markup=get_analytics_keyboard()
        )
