...
def _positions_text(positions):
    ...
    for p in positions:
        ...
        row = (
            f"• {pair} {side}  lev:{lev}  size:{fmt_usd(notional) if notional else '—'}"
        )
        if pnl is not None:
            try:
...
async def cmd_positions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_chat.send_message(
        text, parse_mode="Markdown", reply_markup=kb(rows)
    )
...
async def cb_positions_close(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    svc = get_execution_service(settings)
    ok, msg, res = await svc.execute_close(
        position_index=int(idx), fraction=Decimal(frac)
    )
...
def register(app):
    ...
    app.add_handler(
        CallbackQueryHandler(cb_positions_refresh, pattern=r"^pos:refresh$")
    )
    app.add_handler(
        CallbackQueryHandler(cb_positions_close, pattern=r"^pos:close:\d+:\d+(\.\d+)?$")
    )
