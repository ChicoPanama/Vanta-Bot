...
class RateLimiter:
    ...
    def __init__(self, redis_client: Redis, capacity: int = 10, refill_per_sec: float = 1.0):
...
class CommandRateLimiter:
    ...
    def __init__(self, redis_client: Redis):
        self.redis = redis_client
        self.limiters = {
            'trade': RateLimiter(redis_client, capacity=5, refill_per_sec=0.5),  # 5 trades per 10 seconds
            'status': RateLimiter(redis_client, capacity=20, refill_per_sec=2.0),  # 20 status checks per 10 seconds
            'help': RateLimiter(redis_client, capacity=10, refill_per_sec=1.0),  # 10 help requests per 10 seconds
            'default': RateLimiter(redis_client, capacity=10, refill_per_sec=1.0),  # Default rate limit
        }
    ...
    def allow_command(self, user_id: int, command: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Command rate limiter error for user {user_id}, command {command}: {e}")
            # Allow on error
            return True
