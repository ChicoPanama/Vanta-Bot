...
Examples:
  ...
  python -m src.cli.preflight --collat 100 --lev 5 --slip 0.5 --pair 1 --short
    ...
    parser.add_argument(
        "--pair", type=int, help="Trading pair index (0=BTC, 1=ETH, 2=SOL)"
    )

    # Direction (mutually exclusive)
    direction_group = parser.add_mutually_exclusive_group()
    direction_group.add_argument(
        "--long", action="store_true", help="Validate long position"
    )
    direction_group.add_argument(
        "--short", action="store_true", help="Validate short position"
    )
...
async def validate_trade_params(
    trade_service: TradeService, args: argparse.Namespace
) -> None:
    """Validate specific trade parameters."""
    ...
    try:
        ...
        result = await trade_service.open_market_trade(params=params, dry_run=True)
...
async def check_price_feeds(trade_service: TradeService) -> None:
    ...
    try:
        ...
        pairs = [(0, "BTC/USD"), (1, "ETH/USD"), (2, "SOL/USD")]
