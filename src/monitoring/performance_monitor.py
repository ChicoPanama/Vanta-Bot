...
class PerformanceMonitor:
    ...
    async def _collect_copy_trading_metrics(self):
        ...
        try:
            ...
            await self._store_metric(
                "copy_trades_active_copytraders",
                active_copytraders,
                "gauge",
                {"service": "copy_executor"},
            )
            await self._store_metric(
                "copy_trades_active_follows",
                active_follows,
                "gauge",
                {"service": "copy_executor"},
            )
            await self._store_metric(
                "copy_trades_recent_positions",
                recent_positions,
                "counter",
                {"service": "copy_executor"},
            )
            await self._store_metric(
                "copy_trades_failed_trades",
                failed_trades,
                "counter",
                {"service": "copy_executor"},
            )
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            await self._store_metric(
                "database_connections", db_connections, "gauge", {"service": "database"}
            )

            # Redis connection metrics
            redis_connections = await self._check_redis_health()
            await self._store_metric(
                "redis_connections", redis_connections, "gauge", {"service": "redis"}
            )
    ...
    async def _collect_event_monitoring_metrics(self):
        ...
        try:
            ...
            await self._store_metric(
                "active_traders", active_traders, "gauge", {"service": "event_indexer"}
            )
    ...
    async def _store_metric(
        self, name: str, value: float, metric_type: str, labels: dict[str, str]
    ):
    ...
    async def _write_metrics_to_db(self):
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                logger.debug(
                    f"Persisted {len(self.metrics_buffer)} metrics to database"
                )
            ...
            async with self.db_pool.acquire() as conn:
                ...
                return HealthCheck(
                    ...
                    details={
                        ...
                        "total_traders_analyzed": await conn.fetchval(
                            "SELECT COUNT(DISTINCT address) FROM trader_analytics"
                        ),
                    },
                    checked_at=datetime.utcnow(),
                )
    ...
    async def _send_health_alert(self, health_check: HealthCheck):
        ...
        try:
            ...
            logger.error(
                f"HEALTH ALERT: {health_check.service_name} is {health_check.status.value}"
            )
