...
class AvantisSDKClient:
    ...
    def get_client(self) -> TraderClient:
        ...
        if self._client is None:
            try:
                ...
                logger.info(f"✅ Avantis TraderClient initialized with RPC: {self.rpc_url}")
    ...
    def _setup_signer(self):
        ...
        if private_key:
            ...
            try:
                ...
                logger.info(f"✅ AWS KMS signer configured: {aws_kms_key_id} in {region}")
    ...
    async def ensure_usdc_allowance(self, min_usdc: Union[int, float], trader_addr: str) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Current USDC allowance for {trader_addr}: {current_allowance}")
            ...
            # Increase allowance
            logger.info(f"⚠️ Increasing USDC allowance from {current_allowance} to {min_usdc}")
