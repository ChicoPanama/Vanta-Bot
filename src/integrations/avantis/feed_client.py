    ...
    async def start(self, pair_callbacks: Dict[str, Callable[[PriceUpdate], None]]):
        ...
        try:
            ...
            for pair, callback in pair_callbacks.items():
                self._client.register_price_feed_callback(pair, self._create_price_callback(pair, callback))
            ...
            try:
                ...
                if self._running:
                    ...
                    try:
                        ...
                        logger.error(f"❌ Failed to reconnect FeedClient: {reconnect_error}")
                else:
                    break
    
    def _create_price_callback(self, pair: str, callback: Callable[[PriceUpdate], None]) -> Callable:
        """
