...
class TraderAnalyzer:
    ...
    async def analyze_trader(self, address: str, stats: TraderStats, trades: List[Dict]) -> TraderAnalysis:
    ...
    def _extract_features(self, stats: TraderStats, trades: List[Dict]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            leverage_consistency = 1 - (np.std(leverages) / max(np.mean(leverages), 1)) if leverages else 0
            ...
            return np.array([
                volume_normalized, trade_count_normalized, median_size_normalized,
                win_rate, maker_ratio, avg_hold_time, hold_time_std,
                avg_leverage, leverage_consistency, symbol_entropy,
                long_ratio, size_consistency, 
                *hourly_distribution[:6],  # Top 6 hour preferences
                unique_symbols_ratio,
                stats.realized_pnl_clean_usd / max(stats.last_30d_volume_usd, 1)  # ROI
            ])
    ...
    def _calculate_hold_times(self, trades: List[Dict]) -> List[float]:
        ...
        for trade in sorted(trades, key=lambda x: x['timestamp']):
            ...
            elif trade['event_type'] == 'CLOSED' and key in positions and positions[key]:
                open_time = positions[key].pop(0)
                hold_time = (trade['timestamp'] - open_time).total_seconds() / 3600  # hours
    ...
    def _calculate_risk_metrics(self, trades: List[Dict]) -> Dict[str, float]:
        ...
        try:
            ...
            consistency = 1 - np.std(win_rates_rolling) if len(win_rates_rolling) > 1 else 0
    ...
    def _calculate_recent_volatility(self, trades: List[Dict]) -> float:
        ...
        try:
            ...
            pnls = [t.get('pnl', 0) for t in recent_trades if t['event_type'] == 'CLOSED']
