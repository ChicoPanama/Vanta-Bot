...
def validate_required_secrets() -> None:
    ...
    if os.getenv("REQUIRE_CRITICAL_SECRETS", "true").lower() == "true":
        ...
        required_secrets = ["ENCRYPTION_KEY", "TELEGRAM_BOT_TOKEN", "DATABASE_URL"]

        # Mode-specific secrets
        if signer_backend == "local":
            required_secrets.append("TRADER_PRIVATE_KEY")
        elif signer_backend == "kms":
            required_secrets.extend(["AWS_KMS_KEY_ID", "AWS_REGION"])
        else:
            raise RuntimeError(
                f"Unknown SIGNER_BACKEND: {signer_backend}. Must be 'local' or 'kms'"
            )
