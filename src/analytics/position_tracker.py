...
class PositionTracker:
    ...
    async def _update_trader_stats(self):
        ...
        try:
            ...
            for trader_address in active_traders:
                try:
                    stats = await self._calculate_trader_stats(
                        trader_address, thirty_days_ago
                    )
                    if stats:
    ...
    async def _calculate_trader_stats(
        self, address: str, since: datetime
        ...
        try:
            ...
            # Unique symbols
            unique_symbols = len({trade["pair"] for trade in trades})

            # Last trade time
            last_trade_at = (
                max(trade["timestamp"] for trade in trades) if trades else None
            )
    ...
    def _calculate_lot_pnl(
        self, size: float, entry_price: float, exit_price: float, is_long: bool
    ) -> float:
    ...
    async def get_trader_stats(
        self, address: str, window: str = "30d"
    ) -> Optional[TraderStats]:
            ...
            async with acq as conn:
                ...
                if row:
                    return TraderStats(
                        ...
                        realized_pnl_clean_usd=float(
                            row["realized_pnl_clean_usd"] or 0
                        ),
