...
class AvantisEventIndexer:
    ...
    def _setup_web3(self):
        ...
        try:
            ...
            if os.getenv("BASE_RPC_URL", "").lower() == "memory":
                ...
                logger.info(
                    "Web3 initialized in memory mode; contract filters disabled for tests"
                )
                return
            ...
            with open("config/abis/Trading.json") as f:
                ...
                if not isinstance(trading_abi, list):
                    raise ValueError(
                        "Trading ABI is not a list; invalid ABI file format"
                    )
            ...
            if not addr:
                self.trading_contract = None
                logger.warning(
                    "No trading contract configured; disabling contract filters"
                )
                return
            ...
            try:
                ...
                self.trading_contract = None
                logger.warning(
                    "Invalid contract address '%s'; disabling contract filters", addr
                )
                return

            self.trading_contract = self.web3.eth.contract(
                address=checksum, abi=trading_abi
            )
    ...
    async def _process_block_range(self, from_block: int, to_block: int):
        ...
        try:
            ...
            trade_opened_filter = (
                self.trading_contract.events.TradeOpened.create_filter(
            ...
            trade_closed_filter = (
                self.trading_contract.events.TradeClosed.create_filter(
                    fromBlock=from_block, toBlock=to_block
                )
            )
    ...
    def _normalize_trade_event(self, event, event_type: str) -> TradeEvent:
        ...
        try:
            ...
            price = (
                float(event.args.openPrice) / 1e10
                if hasattr(event.args, "openPrice")
                else 0.0
            )

            # Convert size from DAI (multiplied by 1e18)
            size = (
                float(event.args.positionSizeDai) / 1e18
                if hasattr(event.args, "positionSizeDai")
                else 0.0
            )

            # Get fee if available
            fee = (
                float(event.args.get("fee", 0)) / 1e18
                if hasattr(event.args, "fee")
                else 0.0
            )
    ...
    async def get_trader_events(
        self, address: str, since: Optional[datetime] = None
    ) -> list[TradeEvent]:
