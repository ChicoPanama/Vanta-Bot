...
from src.bot.constants import MAX_LEVERAGE, MAX_POSITION_SIZE, MIN_POSITION_SIZE
    ...
    try:
        ...
        if size < MIN_POSITION_SIZE:
            raise ValidationError(
                f"Trade size too small. Minimum: ${MIN_POSITION_SIZE}"
            )

        if size > MAX_POSITION_SIZE:
            raise ValidationError(
                f"Trade size too large. Maximum: ${MAX_POSITION_SIZE}"
            )
