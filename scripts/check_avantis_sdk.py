...
async def check_sdk_connection():
    ...
    try:
        ...
        if private_key:
            ...
            logger.warning("⚠️ No trader signer configured - trading operations will not work")
            trader_address = None
...
async def check_trade_parameters(client, trader_address):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Opening fee for $100 ETH/USD long @ 10x: {opening_fee} USDC")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"✅ Loss protection: {loss_protection.loss_protection_percent}% "
...
async def check_asset_parameters(client):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ ETH/USD pair spread: bid={pair_spread.bid_price}, ask={pair_spread.ask_price}")
        except Exception as e:
...
async def check_trader_operations(client, trader_address):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Found {len(trades)} active trades and {len(pending_orders)} pending orders")
        except Exception as e:
...
async def main():
    ...
    optional_vars = ['TRADER_PRIVATE_KEY', 'AWS_KMS_KEY_ID', 'AWS_REGION', 'PYTH_WS_URL']
    ...
    for var in optional_vars:
        if os.getenv(var):
            logger.info(f"✅ {var}: {'Set' if var != 'TRADER_PRIVATE_KEY' else 'Set (hidden)'}")
        else:
