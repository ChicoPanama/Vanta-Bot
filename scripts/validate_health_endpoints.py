...
class HealthEndpointValidator:
    ...
    async def test_endpoint(
        self, endpoint: str, timeout: float = 0.5
    ) -> dict[str, Any]:
    ...
    async def test_all_endpoints(self, iterations: int = 10) -> list[dict[str, Any]]:
        ...
        print(
            f"🔍 Testing {len(self.endpoints)} endpoints with {iterations} iterations each"
        )
    ...
    def analyze_results(self, results: list[dict[str, Any]]):
        ...
        for endpoint, endpoint_results in by_endpoint.items():
            ...
            if max_response_time > 0.5:
                print(
                    f"  ⚠️  SLOW RESPONSES: Max {max_response_time:.3f}s > 0.5s threshold"
                )
            ...
            if endpoint in ["/health", "/oracle/status"]:
                ...
                if json_results:
                    ...
                    if endpoint == "/health":
                        ...
                        missing_keys = [
                            k for k in expected_keys if k not in sample_keys
                        ]
                        if missing_keys:
    ...
    async def test_under_load(
        self, concurrent_requests: int = 50, duration_seconds: int = 10
    ):
...
async def main():
    ...
    parser.add_argument(
        ...
        help="Base URL for health endpoints",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--concurrent", type=int, default=50, help="Concurrent requests for load test"
    )
    parser.add_argument(
        "--duration", type=int, default=10, help="Load test duration in seconds"
    )
