...
from src.services.markets.symbols import to_canonical, is_supported_symbol
...
class OracleE2ETester:
    ...
    async def test_symbol_normalization(self):
        ...
        for input_symbol, expected_canonical in test_cases:
            ...
            if result['success']:
                logger.info(f"  ✅ {input_symbol} -> {canonical} (supported: {is_supported})")
            else:
                logger.error(f"  ❌ {input_symbol} -> {canonical} (expected: {expected_canonical}, supported: {is_supported})")
    ...
    async def test_pyth_oracle(self):
        ...
        for symbol in self.test_symbols:
            try:
                ...
                logger.info(f"  ✅ Pyth {symbol}: ${result.price} (dev: {result.deviation_bps}bps, age: {result.freshness_sec}s, {response_time:.3f}s)")
                ...
                if result.freshness_sec > 60:
                    logger.warning(f"  ⚠️  Stale price for {symbol}: {result.freshness_sec}s")
    ...
    async def test_chainlink_oracle(self):
        ...
        try:
            w3 = Web3(Web3.HTTPProvider(self.base_rpc_url, request_kwargs={'timeout': 15}))
            chainlink = ChainlinkOracle(w3, validate_on_init=False)  # Skip startup validation for testing
            ...
            for symbol in self.test_symbols:
                try:
                    ...
                    logger.info(f"  ✅ Chainlink {symbol}: ${result.price} (dev: {result.deviation_bps}bps, age: {result.freshness_sec}s, {response_time:.3f}s)")
        ...
        try:
            ...
            for symbol in self.test_symbols:
                try:
                    ...
                    logger.info(f"  ✅ Facade {symbol}: ${result.price} (dev: {result.deviation_bps}bps, age: {result.freshness_sec}s, {response_time:.3f}s)")
        ...
        try:
            ...
            for symbol in self.test_symbols:
                try:
                    ...
                    deviation_bps = (price_diff / avg_price) * 10000 if avg_price > 0 else 0
                    ...
                    if result_data['success']:
                        logger.info(f"  ✅ {symbol}: Pyth ${pyth_result.price}, Chainlink ${chainlink_result.price} (dev: {deviation_bps:.1f}bps)")
                    else:
                        logger.warning(f"  ⚠️  {symbol}: High deviation {deviation_bps:.1f}bps (Pyth: ${pyth_result.price}, Chainlink: ${chainlink_result.price})")
        ...
        for test_type, test_results in by_test.items():
            ...
            if 'response_time' in test_results[0]:
                ...
                if response_times:
                    ...
                    if max_time > 5.0:
                        logger.warning(f"  ⚠️  Slow responses detected (max: {max_time:.3f}s)")
...
async def main():
    ...
    parser.add_argument('--base-rpc-url', default='https://mainnet.base.org', help='Base RPC URL')
    parser.add_argument('--symbols', nargs='+', default=['BTC/USD', 'ETH/USD', 'SOL/USD'], help='Symbols to test')
