...
def test_database_index():
    ...
    try:
        ...
        cur = con.execute(
            "SELECT user_id FROM user_follow_configs WHERE trader_key=?",
            ("test_trader",),
        )
...
def test_reverse_lookup():
    ...
    try:
        ...
        cur = con.execute(
            "SELECT user_id FROM user_follow_configs WHERE trader_key=?", (trader_key,)
        )
        users = [row[0] for row in cur.fetchall()]
        ...
        cur = con.execute(
            "SELECT DISTINCT trader_key FROM user_follow_configs ORDER BY trader_key"
        )
...
def test_admin_functionality():
    ...
    print(
        f"✅ Admin IDs configured: {len([aid for aid in admin_ids if aid.strip()])} admins"
    )
    ...
    try:
        cur = con.execute(
            "SELECT user_id, trader_key, cfg_json FROM user_follow_configs"
        )
        ...
        auto_copy_count = 0
        for _user_id, _trader_key, cfg_json in configs:
...
def test_fanout_logic():
    ...
    try:
        ...
        print(
            f"✅ Fanout would reach {len(following_users)} users for trader {trader_key}"
        )
