...
class LiveAvantisTrader:
    ...
    def get_wallet_balance(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"   USDC: ~$100 (as reported)")
    ...
    def scale_trade_parameters(self, collateral_usdc: Decimal, leverage: Decimal, 
        ...
        logger.info(f"🔧 SCALING PARAMETERS (Single Scaling)")
    ...
    def run_live_test(self):
        ...
        logger.info(f"🧪 RUNNING LIVE TEST")
        logger.info(f"="*60)
        ...
        if result["success"]:
            ...
            logger.info(f"   Results saved to: live_trade_results.json")
            ...
            logger.info(f"   Error details saved to: live_trade_error.json")
...
def main():
    ...
    try:
        ...
        logger.info("Using correct parameter scaling to bypass SDK double-scaling issues")
        ...
        if result["success"]:
            ...
            logger.info(f"   Bot is now PRODUCTION READY")
            logger.info(f"   Transaction: {result['transaction_hash']}")
            logger.info(f"   BaseScan: {result['basescan_url']}")
            logger.info(f"="*60)
        else:
